...
Param (
    ...
    [string] $credType,
    [Parameter(Mandatory = $true)]
    [string] $robotArtifact, # Input uipath studio msi path or url for msi
    [Parameter()]
    [AllowEmptyString()]
...
Write-Output "importing uipath ps module"
Import-Module UiPath.PowerShell
Write-Output "module imported"

Write-Output "Getting Auth Session"
Get-UiPathAuthToken -URL $orchestratorUrl -Username $adminUsername -Password $orchPassword -TenantName $Tennant -Session
Write-Output "Provision robot and machine to orch"
Add-UiPathRobot -Name $robotname -Username $adminUsername -HostingType $HostingType -MachineName $env:computername #ADDS A ROBOT AND CREATES A MACHINE IF ONE DOESN'T EXISTS 
    ...
    Begin {
        ...
        if (!(Test-Path $robotExePath)) {
            ...
            if ($RobotType -eq "Development") {
                ...
                $msiFeatures = @("DesktopFeature", "Robot", "Studio", "StartupLauncher", "RegisterService", "Packages", "JavaBridge", "ChromeExtension")
            }

            else {
                # log log log
                Log-Write -LogPath $sLogFile -LineValue "Installing UiPath Robot with Studio Feature"
                $msiFeatures = @("DesktopFeature", "Robot", "StartupLauncher", "RegisterService", "Packages", "JavaBridge", "ChromeExtension")
            }
